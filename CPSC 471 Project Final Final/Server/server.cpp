	...
	while(true)
	{
		...
		while((checker = tcp_recv(connfd, tcp_recvbuff, MAX_BUF_SIZE)) > 0){
			...
			if(strcmp("quit\n", tcp_recvbuff) == 0){
				...
				char port[MAX_BUF_SIZE],buffer[MAX_BUF_SIZE],char_numBlocks[MAX_BUF_SIZE],char_nnumLastblock[MAX_BUF_SIZE];
				int datasock,streamSize,numBlocks,nnumLastblock,i;
				FILE *fp;
				token=strtok(NULL," \n");
				printf("Uploading: %s\n ", token);
				...
				if ((fp=fopen(token,"r"))!=NULL)
				{
					...
					streamSize = ftell (fp);
					rewind (fp);
					numBlocks = streamSize/MAX_BUF_SIZE;
					nnumLastblock = streamSize%MAX_BUF_SIZE; 
					sprintf(char_numBlocks,"%d",numBlocks);
					tcp_send(connfd, char_numBlocks, MAX_BUF_SIZE);
					//cout<<numBlocks<<"	"<<nnumLastblock<<endl;
					...
					sprintf(char_nnumLastblock,"%d",nnumLastblock);
					tcp_send(connfd, char_nnumLastblock, MAX_BUF_SIZE);
					if (nnumLastblock > 0) { 
						fread (buffer,sizeof(char),nnumLastblock,fp);
						tcp_send(datasock, buffer, MAX_BUF_SIZE);
						//cout<<buffer<<endl;
					}
					fclose(fp);
				...
				char port[MAX_BUF_SIZE],buffer[MAX_BUF_SIZE],char_numBlocks[MAX_BUF_SIZE],char_nnumLastblock[MAX_BUF_SIZE],check[MAX_BUF_SIZE];
				int datasock,numBlocks,nnumLastblock,i;
				FILE *fp;
				token=strtok(NULL," \n");
				...
				if(strcmp("1",check)==0){
					...
					{
						tcp_recv(connfd, char_numBlocks,MAX_BUF_SIZE);
						numBlocks=atoi(char_numBlocks);
						for(i= 0; i < numBlocks; i++) { 
							tcp_recv(datasock, buffer, MAX_BUF_SIZE);
							fwrite(buffer,sizeof(char),MAX_BUF_SIZE,fp);
						}
						tcp_recv(connfd, char_nnumLastblock, MAX_BUF_SIZE);
						nnumLastblock=atoi(char_nnumLastblock);
						if (nnumLastblock > 0) { 
							tcp_recv(datasock, buffer, MAX_BUF_SIZE);
							fwrite(buffer,sizeof(char),nnumLastblock,fp);
						}
						fclose(fp);
