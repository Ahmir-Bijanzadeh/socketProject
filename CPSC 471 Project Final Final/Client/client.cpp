	...
	*/
	...
	while(fgets(terminal_input, MAX_BUF_SIZE, stdin) != NULL){
		...
		if(strcmp("quit\n", terminal_input) == 0){ // If quit is entered close control connection from client side and close program
			...
			char port[MAX_BUF_SIZE], buffer[MAX_BUF_SIZE],char_numBlocks[MAX_BUF_SIZE],char_numLastblock[MAX_BUF_SIZE],message[MAX_BUF_SIZE];
			FILE *fp;
			...
			if(strcmp("1",message)==0){
				...
				else
				{
					...
					tcp_recv(connfd, char_numLastblock, MAX_BUF_SIZE);
			...
			int dataPort,datasock,streamSize,numBlocks,numLastblock,i;
			char port[MAX_BUF_SIZE], buffer[MAX_BUF_SIZE],char_numBlocks[MAX_BUF_SIZE],char_numLastblock[MAX_BUF_SIZE];
			FILE *fp;
			tcp_recv(connfd, port, MAX_BUF_SIZE);
			dataPort = atoi(port);
			datasock = data_socket(dataPort, argv[1]);
			...
			if((fp = fopen(token,"r")) != NULL){
				...
				numLastblock = streamSize%MAX_BUF_SIZE; 
				...
				tcp_send(connfd, char_numLastblock,  MAX_BUF_SIZE);
				if (numLastblock > 0) { 
