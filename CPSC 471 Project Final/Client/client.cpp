...
int main(int argc, char** argv)
{
	...
	*/
	...
	while(fgets(terminal_input, MAXLINE, stdin) != NULL){
		tcp_send(connfd, terminal_input, MAXLINE); // Send command entered by user to server through control connection.
		char *token, *dummy;
		...
		if(strcmp("quit\n", terminal_input) == 0){ // If quit is entered close control connection from client side and close program
			...
			char buff[MAXLINE], port[MAXLINE]; // Buffers for checks and connection information
			int data_port, datasock; // Variables for socket and port 
			unsigned int temp_size = 1;
			tcp_recv(connfd, port, MAXLINE); // Receiving the port from the server
			...
			while(temp_size != 0){ // Continue to output buff until 
				...
				if( temp_size == 0)
					break;
				tcp_recv(datasock, buff, MAXLINE);
				printf("%s", buff);
			}
